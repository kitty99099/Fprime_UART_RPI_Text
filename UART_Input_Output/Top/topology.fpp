module UART_Input_Output {
  ...
  topology UART_Input_Output {
    ...
    connections UART_Input_Output {
      # Add here connections to user-defined components
    }

    connections UARTConnections {
      # Rate Group 1 (1Hz cycle) ouput is connected to UART's run input
      rateGroup1.RateGroupMemberOut[3] -> UARTInputOutput.run
      # led's gpioSet output is connected to gpioDriver's gpioWrite input
      UARTInputOutput.gpioSet -> gpioDriver.gpioWrite
    }

    connections UART {
      # For reading
      serialDriver.$recv -> UARTInputOutput.UartRead
      UARTInputOutput.UartBuffers -> bufferManager.bufferSendIn
      # For writing
      UARTInputOutput.UartWrite -> serialDriver.$send
      serialDriver.allocate -> bufferManager.bufferGetCallee

      # Deallocate buffer when done
      #serialDriver.deallocate -> bufferManager.bufferSendIn

      #UARTInputOutput.UartBuffers -> serialDriver.$recv
      #serialDriver.$recv -> UARTInputOutput.UartRead
    }
